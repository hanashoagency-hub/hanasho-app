...
function CardForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentIntent?.status === "succeeded") {
      try {
        const res = await fetch("/api/stripe/confirm-cart", {
          method: "POST",
    ...
    // the browser for 3D Secure — /checkout/cart/complete picks it up from there.
...
export default function StripeCartCheckout({
  items,
  onSuccess,
  onError,
}: {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        const res = await fetch("/api/stripe/create-payment-intent-cart", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (data.clientSecret) {
          ...
          setAmount((data.amount || 0) / 100);
        } else {
          setSetupError(data.error || "Could not start card payment.");
        }
      } catch (err) {
        console.error("[stripe] create-payment-intent-cart request failed:", err);
