...
import { LayoutDashboard, BookOpen, Users, CreditCard, LogOut, ArrowLeft, ShieldAlert, Loader2, FileText, Menu, X, PanelLeftClose, PanelLeftOpen, Award, Video, Stamp, BookMarked, Send, Megaphone, TicketPercent, Mail, MessageSquare } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: "/portal-live/messages", label: "Messages", icon: MessageSquare },
  ...
  const getSidebarContent = (isDesktop: boolean) => (
    <>
      <div className={`p-6 border-b border-[var(--border-color)] ${isDesktop && isCollapsed ? 'flex flex-col items-center px-2' : ''}`}>
        <div className={`flex items-center justify-between w-full ${isDesktop && isCollapsed ? 'flex-col gap-4' : ''}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center ${isDesktop && isCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'} rounded-[12px] font-bold text-sm transition-colors ${
                isActive
                  ? "bg-[var(--bg-primary)] text-[var(--text-primary)]"
                  : "text-[var(--text-secondary)] bg-transparent hover:bg-[var(--bg-primary)] hover:text-[var(--text-primary)]"
              }`}
            >
              <item.icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-[var(--brand-primary)]' : 'text-[var(--text-secondary)]'}`} />
              {(!isDesktop || !isCollapsed) && item.label}
            </Link>
          );
        })}
      </nav>
      ...
      <div className={`p-4 border-t border-[var(--border-color)] ${isDesktop && isCollapsed ? 'px-2' : ''}`}>
        <button
          ...
          className={`flex items-center w-full ${isDesktop && isCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'} rounded-[12px] text-red-400 hover:bg-red-500/10 transition-colors font-bold text-sm`}
        >
